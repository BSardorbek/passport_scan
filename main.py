...
img = cv2.imread('images/ale.jpg', 0)
...
(x, y, w, h) = (np.amin(img_hough, axis=0)[0, 0],
                np.amin(img_hough, axis=0)[0, 1],
...
img_roi_copy = cv2.rectangle(img_roi_copy,
                             (x, y - 30),
                             (x + w, y + h),
                             (0, 0, 0),
                             2
...
ret, img_mrz = cv2.threshold(img_mrz,
                             127,
                             255,
                             cv2.THRESH_TOZERO
                             )
